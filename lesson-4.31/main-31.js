...
function calculateFinalPrice(basePrice, discount, taxRate) {
	if (basePrice < 0 || discount < 0 || taxRate < 0) return 0;
...
function checkAccess(name, password) {
	return name === "admin" && String(password) === "123456"
		? "Доступ разрешен"
		: "Доступ запрещен";
}
...
function findFirstEven(startNumber, endNumber) {
...
console.log(findFirstEven(-11, -9));
